  ...
  return (
    <>
      <div className="relative group bg-[#282562] rounded-xl overflow-hidden border border-purple-500/20 hover:border-purple-500/50 transition-all shadow-xl">
        {/* App Screenshot */}
        <div className="relative h-[430px] flex flex-col justify-end">
          <Image 
            src={image}
            alt={appName}
            fill
          ...
          <div className="absolute bottom-0 left-0 p-6 z-10 text-left">
            ...
            <p className="text-gray-200 mt-2 text-sm line-clamp-2 max-w-[280px]">{description}</p>
            
            {/* Tags */}
            <div className="flex flex-wrap gap-2 mt-4">
              {tags.slice(0, 2).map((tag, index) => (
                <span key={index} className="bg-[#523dbe]/40 py-1.5 px-4 rounded-full text-purple-200 text-sm">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <div className="p-4">
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-[#8B5CF6] to-[#EC4899] text-white py-3 px-6 rounded-lg font-medium hover:from-[#7c4fe0] hover:to-[#d9418a] transition-all flex items-center justify-center"
          >
          ...
          <div className="flex gap-3 mt-4">
            {platforms.includes('ios') && (
              <Link 
                ...
                className="flex-1 bg-[#191b3f] border border-gray-800 text-white py-2 px-4 rounded-lg text-sm hover:bg-[#232655] transition-all flex items-center justify-center"
              >
